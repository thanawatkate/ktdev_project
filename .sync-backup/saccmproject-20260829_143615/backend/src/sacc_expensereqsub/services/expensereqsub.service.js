...
async function create(bodyData) {
  ...
  if (!checkTokenEXP(bodyData.token)) { // token valid
    ...
    const refexpense = bodyData.refexpense || bodyData.refExpense;
    let res = {
      status: 'error',
...
// source main ExpenseReq service
async function createInExpenseReqSub(mainID, Data) {
  let bodyData = parseRows(Data);
  let res = false
  for (let index = 0; index < bodyData.length; index++) {
    const row = bodyData[index];
    const result = await db(tableName).insert({
      refexpensereq: mainID,
      refincometype: row.refincometype ?? row.refIncomeType ?? null,
      amount: row.amount,
      remark: row.remark || '',
    });
...
async function updateInExpenseReqSub(mainID, Data) {
...
module.exports = {
  ...
  update,
  remove,
  createInExpenseReqSub,
  updateInExpenseReqSub
}
