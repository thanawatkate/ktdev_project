...
async function create(bodyData) {
  ...
  if (!checkTokenEXP(bodyData.token)) { // token valid
    ...
    const refincome = bodyData.refincome || bodyData.refIncome;
    let res = {
      status: 'error',
...
// source main income service
async function createIncomeSub(mainID, Data, headerCtx, trx = db) {
  const bodyData = JSON.parse(Data);
  const { assertIncomeSubRowsForReporting } = require('../../utils/ledger_subline_validate');
  await assertIncomeSubRowsForReporting(bodyData, headerCtx);
  let res = false;
  for (let index = 0; index < bodyData.length; index++) {
    const row = bodyData[index];
    const rtc = parseInt(String(row.refincometype ?? row.refIncomeType ?? ''), 10);
    const rmt = parseInt(String(row.refmoneytype ?? row.refMoneyType ?? ''), 10);
    const result = await trx(tableName).insert({
      refincome: mainID,
      refincometype: Number.isInteger(rtc) && rtc > 0 ? rtc : null,
      refmoneytype: Number.isInteger(rmt) && rmt > 0 ? rmt : null,
      amount: row.amount,
      remark: row.remark || '',
    });
    if (result[0] > 0 && index === bodyData.length - 1) {
      res = true;
    }
  }
  return res;
...
async function updateIncomeSub(mainID, Data) {
