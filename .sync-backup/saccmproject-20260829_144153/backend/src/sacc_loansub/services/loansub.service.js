...
async function create(bodyData) {
  ...
  if (!checkTokenEXP(bodyData.token)) { // token valid
    ...
    const refloan = bodyData.refloan || bodyData.refLoan;
    let res = {
      status: 'error',
...
async function createLoanSub(mainID, Data) {
...
async function updateLoanSub(mainID, Data) {
