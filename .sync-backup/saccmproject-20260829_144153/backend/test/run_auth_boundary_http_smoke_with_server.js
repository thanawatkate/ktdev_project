...
async function main() {
  ...
  try {
    ...
    await runNodeScript('test/auth_boundary_http_smoke.js');
    console.log('auth-boundary-http-e2e-with-server: PASS');
  } finally {
...
main().catch((err) => {
  console.error('auth-boundary-http-e2e-with-server: FAIL');
